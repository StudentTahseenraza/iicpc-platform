...
class BotEngine {
    async launchBots(targetUrl, botCount, durationSeconds = 30) {
        // targetUrl should already include the correct port
        // Example: ws://localhost:32768
        ...
        for (let i = 0; i < workersCount; i++) {
            const worker = new Worker(path.join(__dirname, 'bot.worker.js'), {
                workerData: {
                    targetUrl,  // This now contains the correct URL with dynamic port
        ...
        console.log(`📊 Test Results: TPS=${tps}, p99=${p99}ms, Error=${errorRate}%, Score=${score}`);
